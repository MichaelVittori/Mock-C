...
func TestNotOperator(t *testing.T) {
	tests := []struct {
		...
		{"!true", false},
		{"!false", true},
		{"!5", false}, // 5 is "truthy" in this language
		{"!!true", true},
		{"!!false", false},
		{"!!5", true},
	}
...
func testEval(input string) (object.Object) {
	...
	env := object.NewEnvironment() // Each test call to eval needs a clean environment
...
func TestIfElseExpressions(t *testing.T) {
	tests := []struct {
		...
		{"if (true) { 10 }", 10},
		{"if (false) { 10 }", nil},
		{"if (1) { 10 }", 10}, // truthy, not necessarily true
		{"if (1 < 2) { 10 }", 10},
		{"if (1 > 2) { 10 }", nil},
...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{
			"5 + true;",
			"Operand type mismatch: INTEGER + BOOLEAN",
		},
		{
			"5 + true; 5;",
			"Operand type mismatch: INTEGER + BOOLEAN",
		...
		{
			"-true",
			"Unsupported negative operand: BOOLEAN",
		},
		{
			"true + false;",
...
func TestArrayLiterals(t *testing.T) {
	...
	if !ok { t.Fatalf("Object is not Array. go=%T (%+v)", evaluated, evaluated) }
	if len(result.Elements) != 3 { t.Fatalf("Array has wrong number of elements. got=%d", len(result.Elements))}
...
func TestHashLiterals(t *testing.T) {
	...
	if !ok { t.Fatalf("Eval didn't return Hash. got=%T (%+v)", evaluated, evaluated) }
	...
	for expectedKey, expectedValue := range expected {
		...
		if !ok { t.Errorf("No pair for given key in Pairs") }
